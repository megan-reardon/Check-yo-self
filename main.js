...
function populateDraftTasks() {
  draftTaskList.insertAdjacentHTML(
    "afterbegin",
...
function populateToDoCard() {
  taskToDoCard.insertAdjacentHTML(
    "afterbegin",
    `<div class="saved-task-cards ${toDoList.id}">
  ...
  if (toDoList.urgent === true) {
    ...
    activeUrgentButton.setAttribute(
      "src",
      "./assets/check-yo-self-icons/urgent-active.svg"
    );
  ...
  for (var i = 0; i < toDoList.tasks.length; i++) {
    ...
    if (toDoList.tasks[i].complete === true) {
      ...
      checkBoxBtn.setAttribute(
        "src",
        "./assets/check-yo-self-icons/checkbox-active.svg"
      );
    } else {
      checkBoxBtn.classList.add("empty-checkbox-img");
      checkBoxBtn.setAttribute(
        "src",
        "./assets/check-yo-self-icons/checkbox.svg"
      );
    }
...
function completeTask(event) {
  ...
  if (event.target.classList.contains("empty-checkbox-img")) {
    ...
    activeCheckBoxBtn.setAttribute(
      "src",
      "./assets/check-yo-self-icons/checkbox-active.svg"
    );
...
function deleteToDoCard(event) {
  ...
  if (event.target.classList.contains("active-delete-btn")) {
    ...
    var deleteButton = event.target
      .closest(".saved-task-cards")
      .querySelector(".delete-btn");
    var deleteButtonDiv = event.target
      .closest(".saved-task-cards")
    ...
    activeDeleteBtn.setAttribute(
      "src",
      "./assets/check-yo-self-icons/delete-active.svg"
    );
...
function markToDoUrgent() {
  if (event.target.closest(".urgent-btn")) {
    var savedToDoCard = event.target.closest(".saved-task-cards");
    var urgentButton = event.target
      .closest(".saved-task-cards")
      .querySelector(".urgent-btn");
    var urgentDiv = event.target
      .closest(".saved-task-cards")
      .querySelector(".task-urgent-btn");
    urgentButton.classList.add("hidden");
    var activeUrgentButton = document.createElement("img");
    activeUrgentButton.classList.add("active-urgent-btn");
